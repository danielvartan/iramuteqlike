...
plot_wordcloud <- function(data, min_freq = 2, max_words = 100,
    ...
    checkmate::assert_integerish(min_freq, lower = 1)
    checkmate::assert_integerish(max_words, lower = 1)
...
#' if (requireNamespace("friends", quietly = TRUE)) {
#'     x <- word_frequency(head(friends::friends[[1]], 100))
#'     plot_word_frequency(x)
#'     plot_word_frequency(x, dist = TRUE)
#' }
#'
#' if (requireNamespace("stringi", quietly = TRUE)) {
#'     x <- word_frequency(stringi::stri_rand_lipsum(5))
...
plot_word_frequency <- function(data, min_freq = 1, max_words = 10,
    ...
    checkmate::assert_string(color, min.len = 1, pattern = "^#")
